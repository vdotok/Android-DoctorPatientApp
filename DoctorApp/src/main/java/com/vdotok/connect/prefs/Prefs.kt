package com.vdotok.connect.prefs
...
import com.vdotok.connect.models.AuthenticationResponse
import com.vdotok.connect.models.GroupModel
import com.vdotok.connect.models.LoginResponse
import com.vdotok.connect.utils.ApplicationConstants.GROUP_MODEL_KEY
import com.vdotok.connect.utils.ApplicationConstants.LOGIN_INFO
import com.vdotok.connect.utils.ApplicationConstants.PRESENCE_MODEL_KEY
import com.vdotok.connect.utils.ApplicationConstants.SDK_AUTH_RESPONSE
import com.vdotok.connect.utils.ApplicationConstants.SOCKET_CONNECTION
...
class Prefs(context: Context?) {
    ...
    fun getGroupList(): List<GroupModel>? {
            val gson = Gson()
            var groupList: List<GroupModel>? = null
            val stringType: String? = mPrefs.getString(GROUP_MODEL_KEY, null)
            val type: Type = object : TypeToken<List<GroupModel>>() {}.type
            stringType?.let { groupList = gson.fromJson(stringType, type)}
            return groupList
    }
